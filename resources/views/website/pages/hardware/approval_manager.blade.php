@extends('website.layouts.main', ['title' => 'Manager Approval Hardware'])
        ...
        <div class="row">
            <div class="card">
                <div class="card-body p-3 table table-responsive">
                    {{-- <a href="{{ route('website.hardware.show_data_manager_approval') }}" class="btn btn-primary">Show Data</a> --}}
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="text" readonly class="form-control-plaintext" id="fullname_form_hardware_reject">
                        <input type="hidden" id="id_form_hardware_reject">
    ...
    <script>
        ...
        $(document).ready(function() {
            var table = $('#app_table').DataTable({
                ...
                // "sScrollY": true,
                ajax: {
                    url: "{{ route('website.hardware.show_manager_approval_ajax') }}",
                },
                columns: [{
            ...
            $('#btn-approve').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('website.hardware.approve_manager') }}",
                    type: "POST",
                    data: {
                        id: id_form_hardware,
                        type: 'ok',
            ...
            $('#btn-reject').on('click', function() {
                let id_form_hardware_reject = $('#id_form_hardware_reject').val();
                console.log(id_form_hardware_reject);
                // window.location.href = "{{ route('website.hardware.approve_manager') }}";
                $.ajax({
                    url: "{{ route('website.hardware.approve_manager') }}",
                    type: "POST",
                    data: {
                        id: id_form_hardware_reject,
                        type: 'reject',
            ...
            $('#app_table').on('click', '.btn-table-reject', function() {
                var id_form_hardware_reject = $(this).data('id');
                var fullname_form_hardware_reject = $(this).data('fullname');
                $('#id_form_hardware_reject').val(id_form_hardware_reject)
                $('#fullname_form_hardware_reject').val(fullname_form_hardware_reject)
                // console.log(id_form_hardware_reject);
            })

@extends('website.layouts.main', ['title' => 'List Guide'])

@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <div class="d-flex justify-content-between">
                <h5 class="card-header">List Guide</h5>
                <a href="{{ route('website.guide.create') }}" class="btn btn-success" style="margin: 1.25rem;">Create</a>
            </div>
    ...
    <script>
        $(document).ready(function() {
            var table = $('#app_table').DataTable({
                ...
                'orderable': true,
                ajax: {
                    url: "{{ route('website.guide.list_ajax') }}",
                },
                columns: [{
                        data: null,
                        orderable: true,
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            return `<div class="text-center">
                                        <a href="{{ url('guide/edit/') }}/${row.uuid}" class="btn btn-sm btn-primary">Edit</a>
                                        <button class="btn btn-danger btn-sm btn-delete" data-bs-toggle="modal" data-bs-target="#deleteModal" data-id="${data.id}" data-name="${data.form_name}">Delete</button>
                                    </div>
                            `;
                        }
                    },
            ...
            $('#btn-approve-delete').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('website.guide.destroy') }}",
                    type: "POST",
                    data: {
                        id: id_delete,
                        '_token': "{{ csrf_token() }}",

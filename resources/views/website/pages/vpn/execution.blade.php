    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <div class="d-flex justify-content-between">
                <h5 class="card-header">PERMIT TO USE VIRTUAL PRIVATE NETWORK (VPN) (FRM-ITD-S13-035-00)</h5>
            </div>
            <div class="row">
                @if (Session::get('info'))
                ...
                <div class="modal-body">
                    ...
                    <div class="form-floating form-floating-outline">
                        <textarea class="form-control auto-resize" id="finish_note_approve" name="finish_note_approve"
                            placeholder="add note if there are additional" style="height: 225px;">{{ old('finish_note_approve') }}</textarea>
    ...
    <script>
        $(document).ready(function() {
            var table = $('#app_table').DataTable({
                ...
                'orderable': true,
                ajax: {
                    url: "{{ route('website.vpn.execution_ajax') }}",
                },
                columns: [{
                        data: null,
                        orderable: true,
                ...
                return (
                    ...
                    <table class="table table-bordered table-sm" style="background-color: #ebf1f2;">
                        <tbody style="border: 2px solid black;">
                            ...
                            <tr>
                                <td style="background-color: #66a7e3; width: 30px; font-weight: bold;">Email</td>
                                <td>${d.email}</td>
                            </tr>
                            <tr>
                                <td style="background-color: #66a7e3; width: 30px; font-weight: bold;">Username</td>
                                <td>${d.username}</td>
                            </tr>
                            <tr>
                        ...
                        <tfoot>
                            <tr>
                                <th colspan="2" class="text-end">
                                    ...
                                    <button class="btn btn-success btn-sm btn-table-approve" data-bs-toggle="modal" data-bs-target="#approveModal" data-id="${d.id}" data-no_reg="${d.no_reg}" data-email="${d.email}" data-username="${d.username}">Approve</button>
                                </th>
                            </tr>    
                        </tfoot>
                    </table>
            ...
            $('#app_table').on('click', '.btn-table-approve', function() {
                ...
                var email_approve = $(this).data('email');
                var username_approve = $(this).data('username');
                ...
                var noteText =
                    'Form VPN telah selesai dibuat. Silahkan login VPN pada Device dengan menginput username bukan email, dengan username : \n\nLogin VPN\nUsername\t: ' +
                    username_approve + '\nPassword\t: (Login Windows anda)\n\n';
            ...
            $('#btn-approve').on('click', function() {
                ...
                let notifikasi_approve = $('#notifikasi_approve').is(':checked') ? 'Ya' :
                    'Tidak';
                $.ajax({
                    url: "{{ route('website.vpn.execution_approve') }}",
                    type: "POST",
                    data: {
                        id: id_approve,

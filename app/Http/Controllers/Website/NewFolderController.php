    ...
    public function manager_approval_ajax(Request $request)
    {
        ...
        $data = NewFolder::where(function ($query) use ($firstDepartmentId, $lastDepartmentId) {
            ...
            ->join('users', 'form_new_folder.created_by', 'users.id')
                        ->leftJoin('users as manager', 'form_new_folder.manager_approve_by', 'manager.id')
                        ->leftJoin('users as it', 'form_new_folder.it_approve_by', 'it.id')
                        ->leftJoin('users as it_mgr', 'form_new_folder.it_mgr_approve_by', 'it_mgr.id')
                        ->leftJoin('users as on_progress', 'form_new_folder.on_progress_by', 'on_progress.id')
                        ->leftJoin('users as finish', 'form_new_folder.finish_by', 'finish.id')

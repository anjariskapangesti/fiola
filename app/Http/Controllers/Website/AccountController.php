...
class AccountController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'npk' => 'nullable|min:6|required_if:form_type,Registration',
            'fullname' => 'required_if:form_type,Registration',
            'department' => 'required_if:form_type,Registration',
            'job_rank' => 'required_if:form_type,Registration',
            'phone' => 'required_if:form_type,Registration',
        ...
        try {
            if($request->form_type == 'Registration') {
            ...
            } elseif ($request->form_type == 'Change') {
                $form_account = Account::create([
                    'no_reg' => $no_reg,
                    'budget_type' => $request->budget_type,
                    'form_type' => $request->form_type,
                    'account_type' => $request->account_type,
                    'npk' => $request->npk_change,
                    'fullname' => $request->fullname_change,
                    'department' => $request->department_change,
                    'job_rank' => $request->job_rank_change,
                    'phone' => $request->phone_change,
                    'company' => $request->company,
                    'expired_date' => $request->expired_date,
                    'purpose' => $request->purpose_change,
                    'ad_name' => $request->ad_name_change,
                    'ad_name_after' => $request->ad_name_after,
                    'is_email' => true,
                    'created_by' => Auth::user()->id,
                    'created_dept' => Auth::user()->departments->pluck('id')->first(),
                    'final_status' => $finalStatus,
            ...
            } elseif ($request->form_type == 'Deletion') {
                $form_account = Account::create([
                    'no_reg' => $no_reg,
                    'budget_type' => $request->budget_type,
                    'form_type' => $request->form_type,
                    'account_type' => $request->account_type,
                    'purpose' => $request->purpose_deletion,
                    'ad_name' => $request->ad_name_deletion,
                    'created_by' => Auth::user()->id,
                    'created_dept' => Auth::user()->departments->pluck('id')->first(),
                    'final_status' => $finalStatus,
                    'is_manager_approve' => $isManagerApprove,
    ...
    {
        return view('website.pages.account.it_mgr_approved');
    }
    ...
    public function execution_approve(Request $request)
    {
        ...
        if ($type == 'approve') {
            $account->email_address = $account->npk . '-aiia@ap01.aisingroup.com';
            $account->is_finish = 1;
            $account->is_confirm = 0;
            ...
            $account->on_progress_by = Auth::user()->id;
            $account->on_progress_date = Carbon::now();
